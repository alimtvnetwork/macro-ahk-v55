...
def apply_replacement(filepath, replacements):
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    for old, new in replacements:
        if old not in content:
            print(f"Failed to find block in {filepath}")
            print("==== EXPECTED ====")
            print(old)
            print("==================")
            sys.exit(1)
...
# SelectorTesterPanel.tsx
target1 = r"""        <div className="flex gap-2">
          <Input
            value={expression}
...
replacement1 = r"""        <SelectorTesterInput expression={expression} setExpression={setExpression} kind={kind} setKind={setKind} />"""

target2 = r"""        {result === null ? (
...
replacement2 = r"""        <SelectorTesterResult result={result} matched={matched} errored={errored} />
      </CardContent>
    </Card>
  );
}
...
apply_replacement("src/components/recorder/SelectorTesterPanel.tsx", [
    (target1, replacement1),
    (target2, replacement2)
])
print("Successfully updated SelectorTesterPanel.tsx")
