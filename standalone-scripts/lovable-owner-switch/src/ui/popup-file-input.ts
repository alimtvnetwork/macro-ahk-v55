/**
 * Owner Switch popup — file picker controller.
 *
 * Wires the file `<input>` change event to `parseOwnerSwitchCsv` and
 * forwards the result to caller-supplied render callbacks. No SQLite,
 * no run logic — pure UI plumbing for P7.
 */
...
const handleFile = async (file: File, onParsed: ParseHandler): Promise<void> => {
    ...
    const result = parseOwnerSwitchCsv(text);
