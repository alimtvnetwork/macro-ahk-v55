/**
 * Owner Switch — minimal RFC 4180 splitter.
 *
 * Handles CR/LF line endings, double-quoted fields, and embedded
 * commas/quotes. No external dependency. Returns rows of raw strings;
 * header normalization happens in `csv-parser.ts`.
 */
...
const pushCell = (row: string[], cell: string): void => {
    row.push(cell);
};
