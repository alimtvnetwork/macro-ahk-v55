...
export class PaymentBannerHider implements PaymentBannerHiderApi {
  ...
  public check(): void {
    try {
      ...
      if (hit.element.getAttribute(STATE_ATTR) !== null) {
        return;
      }
  ...
  private hide(hit: LocateResult): void {
    ...
    for (const t of targets) {
      t.setAttribute(STATE_ATTR, BannerStateType.Fading);
    }

    queueMicrotask(() => {
      for (const t of targets) {
        t.setAttribute(STATE_ATTR, BannerStateType.Hiding);
      }
    });

    window.setTimeout(() => {
      for (const t of targets) {
        t.setAttribute(STATE_ATTR, BannerStateType.Done);
      }
  ...
  private collectCollapseTargets(element: HTMLElement): HTMLElement[] {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (parentEl.tagName === "MAIN" || parentEl.tagName === "BODY") {
        break;
      }
