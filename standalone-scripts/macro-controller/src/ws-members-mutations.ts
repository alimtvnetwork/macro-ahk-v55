...
export async function inviteMember(wsId: string, email: string, role: MemberRole): Promise<void> {
  if (!wsId) {
    throwDiagnostic('WS_MEMBERS_MUTATE_E001', { mutation: 'invite', argument: 'wsId' });
  }

  if (!email) {
    throwDiagnostic('WS_MEMBERS_MUTATE_E001', { mutation: 'invite', argument: 'email' });
  }
...
export async function removeMember(wsId: string, userId: string): Promise<void> {
  if (!wsId) {
    throwDiagnostic('WS_MEMBERS_MUTATE_E001', { mutation: 'remove', argument: 'wsId' });
  }

  if (!userId) {
    throwDiagnostic('WS_MEMBERS_MUTATE_E001', { mutation: 'remove', argument: 'userId' });
  }
...
export async function updateMemberRole(wsId: string, userId: string, role: MemberRole): Promise<void> {
  if (!wsId) {
    throwDiagnostic('WS_MEMBERS_MUTATE_E001', { mutation: 'updateRole', argument: 'wsId' });
  }

  if (!userId) {
    throwDiagnostic('WS_MEMBERS_MUTATE_E001', { mutation: 'updateRole', argument: 'userId' });
  }
