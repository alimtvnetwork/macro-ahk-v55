...
export interface HistoryPanelDeps {
    ...
    toast?: (message: string, kind?: ToastVariantType) => void;
    /**
...
/**
 * PlanTierType 26 step 10 (was v4.185.0 / v4.197.0 dedupe): route History-panel
...
function reportHistoryFailure(
  ...
  context: DiagnosticContext,
  userSentence: string,
  opts: { cause?: unknown; toast?: (m: string, k?: ToastVariantType) => void } = {},
): void {
...
function buildPanel(
  ...
  const renderList = (): void => {
    ...
    for (const rev of view) {
      list.appendChild(buildRevisionRow(doc, rev, role, deps));
    }
...
function buildSubtitleText(total: number, visible: number): string {
  if (total === 0) {
    return 'No revisions yet. Edit this prompt and the previous body will be saved here.';
  }

  if (visible === total) {
    return String(total) + ' revision(s), newest first. Click Restore to bring one back.';
  }
 ...
 */
export type HistoryImportedFilter = HistoryImportedFilterType;
...
export function filterRevisions(
  ...
  return rows.filter((r) => {
    if (filter.reasons.size > 0 && !filter.reasons.has(r.Reason)) {
      return false;
    }
    ...
    if (filter.imported === 'exclude' && isImported) {
      return false;
    }
  ...
  for (let i = 0; i < controls.length; i += 1) {
    ...
    btn.addEventListener('keydown', (ev: KeyboardEvent) => {
      ...
      if (key === 'ArrowRight') {
        ...
        nextIdx = (currentIdx - 1 + controls.length) % controls.length;
      } else if (key === 'Home') {
        nextIdx = 0;
      } else if (key === 'End') {
  ...
  try {
    ...
    setTimeout(() => {
      try {
        ...
        logHistoryDiagnostic('HISTORY_INTERNAL_E001', { stage: 'revoke-url', reason: 'revokeObjectURL failed' }, err); 
      }
    }, 1000);
  } catch (err) {
...
function validateImportEnvelope(
  ...
  if (!parsed || typeof parsed !== 'object') {
    return new DbResult(false, undefined, 'Payload is not an object');
  }

  const p = parsed as Record<string, unknown>;
  if (p['schemaVersion'] !== 1) {
    return new DbResult(false, undefined, 'Unsupported schemaVersion (expected 1, got ' + String(p['schemaVersion']) + ')');
  }

  const pSlug = p['slug'];
  if (typeof pSlug !== 'string' || pSlug.trim() === '') {
    return new DbResult(false, undefined, 'Missing or invalid slug');
  }

  if (pSlug !== expectedSlug) {
    return new DbResult(false, undefined, 'Slug mismatch: file is for "' + pSlug + '", panel is open on "' + expectedSlug + '"');
  }

  const pRole = p['role'];
  if (pRole !== expectedRole) {
    return new DbResult(false, undefined, 'Role mismatch: file is "' + String(pRole) + '", panel is "' + expectedRole + '"');
  }
...
function coerceImportRow(
  ...
  if (!rawItem || typeof rawItem !== 'object') {
    return new DbResult(false, undefined, 'revisions[' + String(index) + '] is not an object');
  }
  ...
  if (typeof rawBody !== 'string' || typeof rawName !== 'string' || typeof rawSlug !== 'string') {
    return new DbResult(false, undefined, 'revisions[' + String(index) + '] missing required fields');
  }

  if (rawSlug !== expectedSlug) {
    return new DbResult(false, undefined, 'revisions[' + String(index) + '] slug mismatch');
  }
...
export function parseRevisionImportPayload(
  ...
  if (envelope.isFail) {
    return new DbResult(false, undefined, envelope.error);
  }
  ...
  for (let i = 0; i < envelope.revisions.length; i += 1) {
    ...
    if (coerced.isFail) {
      return new DbResult(false, undefined, coerced.error);
    }
