...
import { PromptLoadStageType } from "../types/enums";
...
function promptPickerModal(rows: PromptRow[], opts: PickPromptOptions): Promise<PromptRow | null> {
  return new Promise((resolve) => {
    ...
    dlg.style.cssText = 'min-width:340px;max-width:520px;max-height:70vh;display:flex;flex-direction:column;gap:10px;padding:16px;background:hsl(var(--background));border:1px solid hsl(var(--primary) / 0.6);border-radius:8px;box-shadow:0 12px 40px rgba(0,0,0,0.6);color:hsl(var(--foreground));font:12px system-ui,-apple-system,sans-serif;';
    const h = document.createElement('div');
    h.textContent = opts.title;
    h.style.cssText = 'font-size:13px;font-weight:700;color:hsl(var(--primary));';
    dlg.appendChild(h);
    const sel = document.createElement('select');
    sel.size = Math.min(8, Math.max(4, rows.length));
    sel.style.cssText = 'width:100%;padding:6px 8px;background:hsl(var(--background));border:1px solid hsl(var(--border));border-radius:5px;color:hsl(var(--foreground));font-size:12px;';
    for (const r of rows) {
    ...
    cancel.style.cssText = 'padding:6px 12px;background:transparent;border:1px solid hsl(var(--border));border-radius:5px;color:hsl(var(--foreground));cursor:pointer;font-size:12px;';
