...
/**
 * prompt-library-modal - After a successful drag-and-drop import, the polite
 * aria-live status region must announce the import summary so screen-reader
 * users hear the success outcome (e.g. "Import: +1 added, 0 updated").
 */
...
const io = vi.hoisted(() => ({
    ...
    performPromptImport: vi.fn(async () => ({ added: 3, updated: 2, errors: [] })),
}));
...
describe('prompt-library-modal - drop success is announced via aria-live', () => {
    beforeEach(() => {
    ...
    it('the polite status region announces the import summary after a successful drop', async () => {
        ...
        // Structural preconditions on the live region.
        ...
        // screen readers announce the outcome after the drop.
        expect(status.textContent).toContain('Import:');
        expect(status.textContent).toContain('+3 added');
        expect(status.textContent).toContain('2 updated');
    });
});
