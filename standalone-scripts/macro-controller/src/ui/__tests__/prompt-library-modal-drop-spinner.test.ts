/**
 * prompt-library-modal - Spinner + "Importing…" label during drag-and-drop.
 *
 * The Import button label and spinner MUST update for drop-triggered imports
 * exactly as they do for click-triggered ones. Both paths share
 * handleImportFile, so this test proves the drop path also:
 *   - Replaces the button label with "Importing…" while the import runs
 *   - Renders the inline spinner (element with `data-mc-spinner`)
 *   - Restores the original label + removes the spinner when the import
 *     resolves (success) or rejects (failure)
 */
