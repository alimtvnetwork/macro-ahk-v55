...
function legacyBodiesForSlug(slug: string): readonly string[] {
  if (slug === 'next-default') {
    return NEXT_DEFAULT_LEGACY_BODIES;
  }

  if (slug === 'plan-default') {
    return PLAN_DEFAULT_LEGACY_BODIES;
  }
...
function isBundledDefaultDrift(row: typeof PLAN_NEXT_SEED_ROWS[number], body: string): boolean {
  ...
  if (normalizePromptBody(body) === normalizePromptBody(row.body)) {
    return false;
  }

  if (row.slug === 'plan-default') {
    return isManagedPlanDefault(body);
  }

  if (row.slug === 'next-default') {
    return isManagedNextDefault(body);
  }
...
async function upgradeLegacyBodyForRow(row: typeof PLAN_NEXT_SEED_ROWS[number]): Promise<boolean> {
  ...
  if (currentBody === row.body) {
    emitLegacySkip(row, 'already-current');
