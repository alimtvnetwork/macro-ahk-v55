...
function handleSelect(sql: string): { ok: true, isFail: false, isSuccess: true; rows: unknown[] } {
  ...
  const rows = state.prompts.filter((row) => {
    ...
    if (excludeCanonical && row.Slug === CANONICAL_SLUG) {
      return false;
    }

    if (excludeDuplicatePrefix && row.Name.startsWith('[duplicate] ')) {
      return false;
    }
...
vi.mock('../../../src/ui/extension-relay', () => ({
  sendToExtension: vi.fn(async (_channel: string, payload: { method: string; params: { sql: string } }) => {
    ...
    if (payload.method === 'SCHEMA' && /UPDATE\s+Prompt/i.test(sql)) {
      return handleUpdate(sql);
    }

    if (payload.method === 'QUERY' && /SELECT/i.test(sql)) {
