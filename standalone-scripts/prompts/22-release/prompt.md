...
2. **Version Bump**: Bump the MINOR version. `MAJOR.MINOR.PATCH` -> `MAJOR.MINOR+1.0`. `PATCH MUST reset to 0`. **PATCH MUST reset to `0`**.
3. **Canonical source**: The `version.json` at the root is the **canonical version source** and ONLY source of truth.
...
9. **Ambiguity handling**: If any release requirement is unclear, file it in `.lovable/ambiguous-questions/`. Never guess. **Ambiguity handling** is key.
10. **Idempotency guard**: Check if this version was already released. If yes, STOP. **Idempotency guard** is MUST.
11. **Placeholder guard**: Never leave `{{version}}` or `{{date}}` in the final files. **Placeholder guard** is MUST.
