...
if (-not $skippull) {
    ...
    try {
        if (Test-Path ".git") {
            # Prune stale remote refs to avoid "cannot lock ref" errors
            git remote prune origin 2>&1 | Out-Null
            git pull 2>&1 | Out-Host
            if ($LASTEXITCODE -ne 0) {
                Write-Host "  ⚠ git pull failed — attempting fetch + reset..." -ForegroundColor Yellow
                git fetch origin 2>&1 | Out-Null
                $currentBranch = (git rev-parse --abbrev-ref HEAD 2>$null)
                if ($currentBranch) {
                    git reset --hard "origin/$currentBranch" 2>&1 | Out-Host
                    if ($LASTEXITCODE -eq 0) {
                        Write-Host "  ✓ Recovered via fetch + reset to origin/$currentBranch" -ForegroundColor Green
                    } else {
                        Write-Host "  WARNING: recovery failed, continuing with local state..." -ForegroundColor Yellow
                    }
                } else {
                    Write-Host "  WARNING: could not determine branch, continuing..." -ForegroundColor Yellow
                }
            } else {
...
if (-not $skipbuild) {
    ...
    try {
        ...
        # Pre-build guard: verify standalone dist artifacts if configured
        # Mirrors scripts/check-standalone-dist.mjs — prevents building against stale/missing bundles
        if ($Config.standaloneArtifacts) {
            Write-Host "  Verifying standalone dist artifacts..." -ForegroundColor Gray
            $standaloneBase = Resolve-RelativePath ($Config.standaloneArtifacts.baseDir)
            $saGuardFailed = $false
            foreach ($entry in $Config.standaloneArtifacts.required) {
                $saDistPath = Join-Path $standaloneBase "$($entry.folder)/dist"
                if (-not (Test-Path $saDistPath)) {
                    Write-Host "  [FAIL] $($entry.folder)/dist/ does not exist" -ForegroundColor Red
                    $saGuardFailed = $true
                    continue
                }
                foreach ($file in $entry.files) {
                    $saFilePath = Join-Path $saDistPath $file
                    if (-not (Test-Path $saFilePath)) {
                        Write-Host "  [FAIL] $($entry.folder)/dist/$file is missing" -ForegroundColor Red
                        $saGuardFailed = $true
                    } elseif ((Get-Item $saFilePath).Length -lt 100) {
                        Write-Host "  [FAIL] $($entry.folder)/dist/$file is suspiciously small ($((Get-Item $saFilePath).Length) bytes)" -ForegroundColor Red
                        $saGuardFailed = $true
                    }
                }
            }
            if ($saGuardFailed) {
                Write-Host "  [FAIL] Standalone dist artifacts missing or stale. Build them first." -ForegroundColor Red
                exit 3
            }
