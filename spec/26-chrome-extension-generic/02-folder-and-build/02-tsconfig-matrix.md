...
Document the multi-tsconfig pattern: tsconfig.json (root references), tsconfig.app.json (extension UI), tsconfig.sdk.json (page-injected SDK), tsconfig.node.json (build scripts), tsconfig.macro.json / tsconfig.xpath.json (per-standalone-bundle). Explain composite project refs and why each split exists.

---

## Source material to distill (private — do NOT cite in final body)

`tsconfig*.json` files in repo root
