...
Per-bundle vite configs (vite.config.ts for the extension, vite.config.sdk.ts and one per standalone). Cover IIFE output for page-injected scripts, source-map strategy (inline in dev, none in prod), writeBundle hooks, static top-level Node imports rule.

---

## Source material to distill (private — do NOT cite in final body)

`vite.config*.ts` files, mem://architecture/sourcemap-strategy, mem://constraints/vite-build-environment
