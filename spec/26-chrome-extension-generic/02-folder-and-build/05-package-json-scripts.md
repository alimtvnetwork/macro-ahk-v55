...
List the canonical npm script set (build, build:sdk, build:standalone-*, dev, test, lint, package, validate). Provide a minimum-viable dep list (vite, react, typescript, tailwind, eslint + plugins, vitest, playwright, sql.js, etc.) with version policy notes.

---

## Source material to distill (private — do NOT cite in final body)

`package.json`, `scripts/check-version-sync.mjs`
