...
Provide the full eslint.config.js template (flat config). Cover: typescript-eslint, sonarjs, react-hooks, import, custom rules (no-const-reassign, no-bun-in-scripts). Explain rule severity choices.

---

## Source material to distill (private — do NOT cite in final body)

`eslint.config.js`, `scripts/lint-const-reassign.mjs`, `scripts/no-bun-in-scripts.mjs`, mem://architecture/linting-policy
