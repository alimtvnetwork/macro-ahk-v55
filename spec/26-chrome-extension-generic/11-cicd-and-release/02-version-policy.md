...
Mandate: a single version string across manifest.json, src/shared/constants.ts, and each standalone SDK. Bump policy: every release ticks the patch (or higher). Validation: check-version-sync.mjs MUST be green.

---

## Source material to distill (private — do NOT cite in final body)

mem://workflow/versioning-policy, mem://workflow/automated-version-validation
