...
5) No Code Squashing (RCA Note): Never compress code to meet line count budgets. Every statement must reside on its own line (`max-statements-per-line: 1`). Any code compressed, squashed, or written as one-liners is classified as 'garbage' and strictly prohibited.
6) Multi-line Functions: Even simple arrow functions or one-liners must use `{}` braces and span multiple lines, exactly adhering to standard multi-line formatting. Do not collapse them onto a single line. If a function or file exceeds length limits, refactor by extracting helper functions or creating new files.
