...
Document the ERROR_COUNT_CHANGED message pattern that broadcasts error-state changes across extension contexts so the UI badge stays in sync in real time.

---

## Source material to distill (private — do NOT cite in final body)

mem://architecture/real-time-error-synchronization, `src/hooks/use-error-count.ts`
