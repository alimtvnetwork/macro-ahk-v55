...
Document the dist/ folder contract (what files MUST be present, naming, version.txt), the nix-zip packaging recipe, and the release ZIP layout. Include the validation scripts that run before any release.

---

## Source material to distill (private — do NOT cite in final body)

`scripts/check-standalone-dist.mjs`, `scripts/check-sdk-dist-freshness.mjs`, `scripts/check-xpath-dist-freshness.mjs`
