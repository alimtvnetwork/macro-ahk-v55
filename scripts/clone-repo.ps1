...
[CmdletBinding()]
param(
    [string]$Repo = $(if ($env:MARCO_REPO_CLONE_REPO) { $env:MARCO_REPO_CLONE_REPO } else { 'alimtvnetwork/macro-ahk-v53' }),
...
function Normalize-Repo([string]$RawRepo) {
    ...
    if ($repoValue -ieq 'alimtvnetwork/macro-ahk-v53') {
        Write-Warn "Stale repo owner replaced: alimtvnetwork/macro-ahk-v53 -> alimtvnetwork/macro-ahk-v53"
        return 'alimtvnetwork/macro-ahk-v53'
    }
