...
function getNewName(oldName: string, values: string[]): string {
    if (oldName.endsWith("Type")) return oldName;
    
    let baseName = oldName;
    
    // Handle some specific weird ones
    if (baseName === "SemanticSemanticToneEnum1") return "SemanticToneExtendedType";
    if (baseName === "SemanticSemanticKindEnum1") return "SemanticKindExtendedType";
    if (baseName === "ApplyQuotaResultResult") return "ApplyQuotaResultType";
    if (baseName === "ApplyErrorRateResultResult") return "ApplyErrorRateResultType";
        ...
        if (initializer) {
            ...
            for (const prop of properties) {
                if (prop.getKind() === SyntaxKind.PropertyAssignment) {
                    ...
                    if (init) values.push(init.getLiteralValue());
                }
            }
