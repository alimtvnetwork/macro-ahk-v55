...
function buildShell(): ModalRefs {
  ...
  // Polite live region: success/progress messages (ImportingΓÇª, summary,
...
async function handleImportFile(
  ...
  if (invalid) {
    ...
    // button is offered ΓÇö retrying the same rejected file would just fail
...
/**
 * Swap the Import button label for an inline spinner + "ImportingΓÇª" while the
...
function showImportSpinner(importBtn: HTMLButtonElement): void {
  ...
  importBtn.appendChild(importBtn.ownerDocument.createTextNode('ImportingΓÇª'));
}
...
function buildImportProgressElement(): {
  ...
  label.textContent = 'Preparing importΓÇª';
...
function showImportProgress(refs: ModalRefs): void {
  ...
  p.label.textContent = 'Preparing importΓÇª';
...
async function buildRoleSection(refs: ModalRefs, role: PromptRole): Promise<HTMLElement> {
  ...
  for (const row of sortedRows) {
    wrap.appendChild(buildRowContainer(refs, row));
  }
...
function buildRowLeft(refs: ModalRefs, row: PromptRow, container: HTMLElement): HTMLElement {
  ...
  left.addEventListener('click', () => {
    togglePreview(refs, row, container); 
  });
  const isExpanded = refs.view.expandedIds.has(row.Id);
  const caret = isExpanded ? 'Γû╛ ' : 'Γû╕ ';
  const name = document.createElement('div');
  name.textContent = caret + (row.IsDefault ? 'Γÿà ' : '') + row.Name;
...
function buildRowRight(refs: ModalRefs, row: PromptRow, rowEl: HTMLElement): HTMLElement {
  ...
  setDefaultBtn.addEventListener('click', () => {
    void handleSetDefault(refs, row); 
  });
  ...
  editBtn.addEventListener('click', () => {
    openInlineEditor(refs, rowEl, row); 
  });
  ...
  if (canReset) {
    ...
    resetBtn.addEventListener('click', () => {
      void handleResetToDefault(refs, row); 
    });
...
async function handleResetToDefault(refs: ModalRefs, row: PromptRow): Promise<void> {
  ...
  try {
    ...
    showToast('Γå║ Reset to default: ' + row.Name, 'success');
    await renderAllRoles(refs);
  } catch (err) {
