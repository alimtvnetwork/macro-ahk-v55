...
function isStringLiteralUnion(node: Node): boolean {
    ...
    for (const t of types) {
        ...
        if ((t as LiteralTypeNode).getLiteral().getKind() !== SyntaxKind.StringLiteral) return false;
    }
    return true;
}
...
function generateNameForUnion(node: Node, values: string[]): string | null {
    ...
    if (joined === "asc_desc") return "SortOrder";
    if (joined === "replace_toggle_range") return "SelectMode";
    
    return "Enum_" + Math.abs(joined.split("").reduce((a, b) => { a = ((a << 5) - a) + b.charCodeAt(0); return a & a }, 0)).toString(16);
}
...
for (const sourceFile of project.getSourceFiles()) {
    if (sourceFile.getFilePath().includes('types/enums.ts')) continue;
    ...
    const inlineUnions = unions.filter(u => {
        if (!isStringLiteralUnion(u)) return false;
    ...
    for (const union of inlineUnions) {
        ...
        if (values.length < 2) continue; // Not a real union enum
