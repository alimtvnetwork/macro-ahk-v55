...
describe("useKeywordEvents, reorderEvents", () => {
  ...
  function seedThree() {
    ...
    let cId = "";
    act(() => {
      aId = result.current.addEvent("alpha"); 
    });
    act(() => {
      bId = result.current.addEvent("beta"); 
    });
    act(() => {
      cId = result.current.addEvent("gamma"); 
    });
  ...
  it("moves an event from the start to the end", () => {
    ...
    act(() => {
      result.current.reorderEvents(aId, cId); 
    });
  ...
  it("moves an event from the end to the start", () => {
    const { result, aId, cId } = seedThree();
    act(() => {
      result.current.reorderEvents(cId, aId); 
    });
  ...
  it("is a no-op when fromId === toId", () => {
    ...
    act(() => {
      result.current.reorderEvents(bId, bId); 
    });
  ...
  it("ignores unknown ids (stale drag from a removed row)", () => {
    ...
    const before = result.current.events.map(e => e.Id);
    act(() => {
      result.current.reorderEvents(aId, "ghost-id"); 
    });
    act(() => {
      result.current.reorderEvents("ghost-id", aId); 
    });
    expect(result.current.events.map(e => e.Id)).toEqual(before);
  });
  ...
  it("preserves each event's steps and metadata after reorder", () => {
    ...
    act(() => {
      result.current.addStep(aId, { Kind: "Key", Combo: "Enter" } as Omit<import("@/hooks/use-keyword-events").KeywordEventStep, "Id">); 
    });
    act(() => {
      result.current.addStep(bId, { Kind: "Wait", DurationMs: 250 } as Omit<import("@/hooks/use-keyword-events").KeywordEventStep, "Id">); 
    });
