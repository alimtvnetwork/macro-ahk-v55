...
import { SqlStageType, DirectionType } from "../../../standalone-scripts/macro-controller/src/types/enums";
...
export interface BootstrapDeps {
    ...
    readonly classifyLoadError: (err: unknown, stage: SqlStageType) => StepLibraryLoadError;
...
function applyRemoteBytes(deps: RemoteSyncDeps, remoteBytes: Uint8Array): void {
  if (!deps.lib || !deps.project || !deps.sql) {
    return;
  }
...
function useAfterMutation(deps: MutationDeps, commit: () => void) {
  return useCallback(() => {
    if (deps.lib === null || deps.project === null) {
      return;
    }
