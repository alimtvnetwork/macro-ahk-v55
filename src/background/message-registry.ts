...
export const HANDLER_REGISTRY = new Map<MessageType, MessageHandler>([
  ...
  [MessageType.CLEAR_NETWORK_REQUESTS, async () => {
  ...
  [MessageType.RESEED_PROMPTS, async () => {
    await reseedPrompts();
  ...
  [MessageType.DELETE_UPDATER, async (payload) => {
    handleDeleteUpdater((payload as Record<string, unknown>).updaterId as number);
  ...
  [MessageType.SAVE_UPDATE_SETTINGS, async (payload) => {
    handleSaveUpdateSettings((payload as Record<string, unknown>).data as Record<string, unknown>);
