...
function inferCategory(type: MessageType): string {
  ...
  if (type.includes("SCRIPT") || type.includes("CONFIG")) {
...
function inferDescription(type: MessageType): string {
  if (type.startsWith("GET_")) {
    return `Reads data using ${type}.`;
  }

  if (type.startsWith("SAVE_")) {
    return `Creates or updates data using ${type}.`;
  }

  if (type.startsWith("DELETE_")) {
    return `Deletes data using ${type}.`;
  }

  if (type.startsWith("CLEAR_")) {
    return `Clears state/data using ${type}.`;
  }
