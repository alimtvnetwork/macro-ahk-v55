...
function uniqueRenameFor(
  ...
  if (!existingLower.has(desired.toLowerCase())) {
    return desired;
  }
...
function checkRunGroupTargets(graph: SourceGraph): ImportFailure | null {
  if (graph.danglingRunGroupIds.length === 0) {
    return null; 
  }
...
function resolveRootNameConflict(
  ...
  if (conflict === "Fail") {
    return { Kind: "Collision", Name: g.Name }; 
  }

  if (conflict === "Skip") {
    return { Kind: "Skip", Name: g.Name }; 
  }
...
function insertGroups(
  ...
  for (const g of graph.sourceGroups) {
    if (plan.skippedSubtree.has(g.StepGroupId)) {
      continue;
    }
