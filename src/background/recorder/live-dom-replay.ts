...
import { ReplayStepInputKindType, OnTimeoutType, PredicateEvaluationKindType, WaitForOutcomeReasonType } from "../../types/enums";
...
export interface ReplayStepInput {
    ...
    readonly Kind: ReplayStepInputKindType;
...
export interface StepGate {
    ...
    readonly OnTimeout: OnTimeoutType;
}
...
function actuateStep(step: ReplayStepInput, target: HTMLElement, row: FieldRow | undefined): void {
  ...
  if (step.Kind === "Type")   {
    dispatchType(target,   resolveValue(step.Value, row));

    return; 
  }

  if (step.Kind === "Select") {
    dispatchSelect(target, resolveValue(step.Value, row)); 
  }
}
...
function buildWaitFailure(
  ...
  waitOutcome: { Ok: false; Reason: WaitForOutcomeReasonType; DurationMs: number; Detail?: string },
): ReplayStepResult {
...
function finalize(
  ...
  if (outcome.Ok) {
    return buildSuccessResult(step, started, finished, outcome); 
  }
