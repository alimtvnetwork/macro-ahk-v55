...
function walkCondition(node: Condition, depth: number, path: string, state: WalkState): void {
  ...
  if ("Not" in node) {
    walkCondition(node.Not, depth + 1, joinPath(path, "Not"), state);
...
export function evaluateCondition(condition: Condition, options: EvaluateOptions): boolean {
  if ("All" in condition) {
    for (const child of condition.All) {
      if (evaluateCondition(child, options) === false) {
        return false;
      }
    }
  ...
  if ("Any" in condition) {
    for (const child of condition.Any) {
      if (evaluateCondition(child, options)) {
        return true;
      }
    }
  ...
  if ("Not" in condition) {
    return evaluateCondition(condition.Not, options) === false;
  }
