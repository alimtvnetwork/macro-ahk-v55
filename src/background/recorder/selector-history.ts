...
import { SelectorHealthType } from "../../types/enums";
...
export type SelectorHealth = SelectorHealthType;
...
function findLastSuccess(
  ...
  for (let i = outcomes.length - 1; i >= 0; i--) {
    if (outcomes[i].IsOk) {
      return { lastSuccessAt: outcomes[i].At, lastSuccessIdx: i }; 
    }
  }
...
function classifyStatus(
  ...
  if (resolved === null && totals.totalRuns === 0) {
    return "unknown"; 
  }
  ...
  if (totals.totalFailures === totals.totalRuns) {
