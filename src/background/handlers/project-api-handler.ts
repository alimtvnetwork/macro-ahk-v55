...
type RawSqlKind = RawSqlKindType;
...
function classifyStatement(statement: string): RawSqlKind {
  ...
  if (/^pragma\s+table_info\s*\(/.test(normalized)) {
    return "read";
  }

  if (/^(insert|update|delete)\b/.test(normalized)) {
    return "write";
  }

  if (/^create\s+table\b/.test(normalized)) {
    return "schema";
  }

  if (/^create\s+(unique\s+)?index\b/.test(normalized)) {
    return "schema";
  }

  if (/^create\s+view\b/.test(normalized)) {
    return "schema";
  }

  if (/^drop\s+view\b/.test(normalized)) {
    return "schema";
  }
  ...
  if (/^begin(\s+transaction)?\b/.test(normalized)) {
    return "transaction";
  }

  if (/^(commit|rollback)\b/.test(normalized)) {
    return "transaction";
  }
