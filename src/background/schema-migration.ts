...
function inferSchemaVersion(logsDb: SqlJsDatabase, errorsDb: SqlJsDatabase): number {
  ...
  if (sessionsColumns.has("StartedAt")) {
    if (logsTables.has("AssetVersion")) {
      return 8; 
    }

    if (logsTables.has("SharedAsset") || logsTables.has("ProjectGroup")) {
      return 7; 
    }

    if (logsTables.has("UpdateSettings")) {
      return 6; 
    }

    if (logsTables.has("UpdaterInfo")) {
      return 5; 
    }

    if (errorsTables.has("ErrorCodes")) {
      return 4; 
    }
