...
function projectBindsScript(
  ...
  if (script.filePath) {
    scriptKeys.add(basename(script.filePath));
  }

  return (project.scripts ?? []).some((entry) => {
    if (entry.path === script.id) {
      return true;
    }

    if (entry.path === script.name) {
      return true;
    }
...
function collectPatternsForScript(script: StoredScript, projects: StoredProject[]): Set<string> {
  ...
  for (const project of projects) {
    if (!projectBindsScript(project, script)) {
      continue;
    }
...
export async function backfillScriptUrlMatches(): Promise<BackfillResult> {
  ...
  for (const script of scripts) {
    if (processScript(script, projects, result)) {
      mutated = true; 
    }
  }
