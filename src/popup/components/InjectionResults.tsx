import { InjectionSkipReasonType } from "../../../standalone-scripts/macro-controller/src/types/enums";
...
export interface InjectionResultEntry {
    ...
    errorMessage?: string;
    durationMs?: number;
    skipReason?: InjectionSkipReasonType;
    scriptName?: string;
}
...
function getResultIcon(result: InjectionResultEntry): string {
  ...
  if (result.skipReason === "missing") {
    return "🔍";
  }

  if (result.skipReason === "resolver_mismatch") {
    return "⚠️";
  }
...
function getRowClass(result: InjectionResultEntry): string {
  if (result.isSuccess) {
    return "injection-result-row";
  }

  if (result.skipReason) {
    return "injection-result-row injection-result-row--skipped";
  }
