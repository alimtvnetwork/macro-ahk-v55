...
function applySelection(
    setSelected: UseLibrarySelectionArgs["setSelected"],
    setSelectionOrder: UseLibrarySelectionArgs["setSelectionOrder"],
    on: boolean,
...
export function useLibrarySelection(args: UseLibrarySelectionArgs) {
    ...
    return {
        toggleOne: (id: number, on: boolean) => applySelection(setSelected, setSelectionOrder, on, [id]),
        toggleSubtree: (node: TreeNode, on: boolean) => {
            const ids = new Set<number>();
            collectDescendantIds(node, ids);
            applySelection(setSelected, setSelectionOrder, on, Array.from(ids));
        },
        clearSelection: () => {
