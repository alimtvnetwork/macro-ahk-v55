...
function useSelectionToggles(setSelected: React.Dispatch<React.SetStateAction<ReadonlySet<number>>>, visibleIds: number[]) {
    ...
    const toggleAllVisible = (on: boolean) => {
        setSelected((prev) => {
            const next = new Set(prev);
            if (on) for (const id of visibleIds) next.add(id); else for (const id of visibleIds) next.delete(id);
...
export function useListPanelSelection(
    ...
    const toggles = useSelectionToggles(setSelected, visibleIds);
