...
function useHandleFile(setText: (text: string) => void) {
...
function useGroupInputFileHandlers(setText: (text: string) => void) {
  const [dragOver, setDragOver] = useState(false);
  const handleFile = useHandleFile(setText);
  ...
  return { dragOver, setDragOver, handleFilePick, handleDrop };
}

// eslint-disable-next-line max-lines-per-function
function useGroupInputActions(args: UseGroupInputsControllerArgs, setText: (t: string) => void, parseResult: ReturnType<typeof parseGroupInputJson>) {
  const { groupId, groupName, onApply, onClear, onOpenChange } = args;
  const { toast } = useToast();
...
export function useGroupInputsController(args: UseGroupInputsControllerArgs) {
  const { open, currentBag } = args;
  const [text, setText] = useState("");
  const { dragOver, setDragOver, handleFilePick, handleDrop } = useGroupInputFileHandlers(setText);
  const parseResult = useMemo(() => parseGroupInputJson(text), [text]);
  const { handleApply, handleClear } = useGroupInputActions(args, setText, parseResult);
  ...
  }, [open, currentBag, setDragOver]);

  const handleLoadCurrent = useCallback(() => {
    setText(currentBag === null ? "" : JSON.stringify(currentBag, null, 2));
  }, [currentBag]);

  return { text, setText, dragOver, setDragOver, parseResult, handleLoadCurrent, handleFilePick, handleDrop, handleApply, handleClear };
}
