...
export function presentVariant(entry: WebhookDeliveryResult): VariantPresentation {
  if (isWebhookSuccess(entry)) {
    return presentSuccess(entry);
  }

  if (isWebhookSkipped(entry)) {
    return presentSkipped(entry);
  }
...
function variantHeader(entry: WebhookDeliveryResult): string {
  if (isWebhookSkipped(entry)) {
    return "[SKIPPED] Webhook Delivery";
  }

  if (isWebhookSuccess(entry)) {
    return "[SUCCESS] Webhook Delivery";
  }
...
function variantStatusBlock(entry: WebhookDeliveryResult): string {
  if (isWebhookSkipped(entry)) {
    return describeSkipped(entry);
  }

  if (isWebhookSuccess(entry)) {
    return describeSuccess(entry);
  }

  if (isWebhookFailure(entry)) {
    return describeFailure(entry);
  }
