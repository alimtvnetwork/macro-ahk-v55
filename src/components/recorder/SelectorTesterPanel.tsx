...
function elementSummary(el: DomContext | null): string {
  ...
  if (el.ClassName !== null) {
    attrs.push(`.${el.ClassName.split(/\s+/).filter(Boolean).join(".")}`);
  }
...
export function SelectorTesterPanel({ doc }: SelectorTesterPanelProps = {}) {
  ...
  const result = useMemo(() => {
    if (targetDoc === null || expression.trim().length === 0) {
      return null;
    }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        <SelectorTesterInput expression={expression} setExpression={setExpression} kind={kind} setKind={setKind} />
        ...
        <SelectorTesterResult result={result} matched={matched} errored={errored} />
      </CardContent>
    </Card>
  );
}
