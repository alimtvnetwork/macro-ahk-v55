...
function TesterInputArea({
    expression, setExpression, kind, setKind, detectedKind,
}: {
    expression: string; setExpression: (v: string) => void; kind: SelectorTestKind; setKind: (v: SelectorTestKind) => void; detectedKind: SelectorTestKind | null;
}) {
    return (
        <>
...
function TesterResultDisplay({
    result, errored, matched,
}: {
    result: ReturnType<typeof testSelector> | null; errored: boolean; matched: boolean;
}) {
    if (result === null) {
        return (
