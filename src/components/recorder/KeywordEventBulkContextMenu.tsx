...
export function KeywordEventBulkContextMenu(
  ...
  const handleEnable = (enabled: boolean): void => {
    for (const ev of selectedEvents) {
      onUpdateEvent(ev.Id, { Enabled: enabled });
    }
  };
...
export function BulkRenameSequenceDialog(props: BulkRenameSequenceDialogProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    const handler = (e: StorageEvent): void => {
      if (e.key !== SEQUENCE_RENAME_STORAGE_KEY) {
        return;
      }
...
function PreviewSummaryBanner({ summary }: { readonly summary: ReturnType<typeof computeSequencePreview> }): JSX.Element | null {
  ...
  if (summary.CollisionCount > 0) {
    parts.push(`${summary.CollisionCount} collide with existing names`);
  }

  if (summary.EmptyCount > 0)     {
    parts.push(`${summary.EmptyCount} empty`);
  }

  if (summary.TooLongCount > 0)   {
    parts.push(`${summary.TooLongCount} too long`);
  }
