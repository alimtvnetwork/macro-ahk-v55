...
function DriftTimelineStrip({ timeline }: { timeline: DriftTimeline }) {
    ...
    if (State === "healthy" && LastSuccess !== null) {
        return <HealthyTimeline lastSuccess={LastSuccess} />;
    }

    if (State === "always-failing" && FirstDrift !== null) {
        return <AlwaysFailingTimeline firstDrift={FirstDrift} failures={FailuresSinceDrift} />;
    }
...
function HealthyTimeline({ lastSuccess }: { lastSuccess: NonNullable<DriftTimeline["LastSuccess"]> }) {
    return (
...
function AlwaysFailingTimeline({ firstDrift, failures }: { firstDrift: NonNullable<DriftTimeline["FirstDrift"]>; failures: number }) {
    return (
        <div className="flex items-center gap-2 rounded-md border border-destructive/40 bg-destructive/5 px-3 py-2 text-xs">
            ...
            <span className="text-muted-foreground">
                ...
                {failures > 1 && ` · ${failures} failures total`}
            </span>
...
function DriftedTimeline({
    lastSuccess,
    firstDrift,
    healthyDurationMs,
    failures,
}: {
    lastSuccess: NonNullable<DriftTimeline["LastSuccess"]>;
    firstDrift: NonNullable<DriftTimeline["FirstDrift"]>;
    ...
    return (
        <div className="rounded-md border border-amber-500/40 bg-amber-500/5 px-3 py-2 text-xs space-y-1.5">
            <DriftedTimelineHeader healthyDurationMs={healthyDurationMs} />
            <div className="flex items-center gap-2 font-mono">
                <TimelineSuccessNode node={lastSuccess} />
                <ArrowRight className="h-3 w-3 text-muted-foreground" aria-hidden />
                <TimelineDriftNode node={firstDrift} />
                {failures > 1 && (
                    <span className="ml-auto text-[10px] text-muted-foreground">
                        +{failures - 1} more failure{failures === 2 ? "" : "s"}
                    </span>
                )}
            </div>
...
function DriftedTimelineHeader({ healthyDurationMs }: { healthyDurationMs: number | null }) {
    return (
...
function TimelineSuccessNode({ node }: { node: NonNullable<DriftTimeline["LastSuccess"]> }) {
    return (
        <span
...
function TimelineDriftNode({ node }: { node: NonNullable<DriftTimeline["FirstDrift"]> }) {
    return (
        <span
