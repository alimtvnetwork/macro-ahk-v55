...
```powershell
$env:MARCO_DL_VERSION='v5.10.0'; $env:MARCO_DL_FOLDER='marco'; irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v55/main/scripts/download-extension.ps1 | iex
```

**Windows · PowerShell — run a local clone with explicit flags:**

```powershell
.\scripts\download-extension.ps1 -Version v5.10.0 -FolderName marco-extension
```
...
After it finishes, load the resulting folder via `chrome://extensions → Load unpacked`. The `marco-extension-v5.10.0.zip` backup sits next to it.
...
```powershell
# Windows · PowerShell
irm https://github.com/alimtvnetwork/macro-ahk-v55/releases/download/v5.10.0/install.ps1 | iex
...
```bash
# macOS · Linux · Bash
curl -fsSL https://github.com/alimtvnetwork/macro-ahk-v55/releases/download/v5.10.0/install.sh | bash


```

**Pinned version:** `v5.10.0` &nbsp;•&nbsp; **Macro Controller:** `v5.10.0`
...
- 🔒 Pinned to `v5.10.0` — see [Pinned to a specific release](#-pinned-to-a-specific-release) to change versions
...
- 🔒 Pinned to `v5.10.0` — see [Pinned to a specific release](#-pinned-to-a-specific-release) for other tags
...
|------|-------------|---------|
| `-Version` | Pin a specific release (`vX.Y.Z[-pre]`) or `latest` | `-Version v5.10.0` |
